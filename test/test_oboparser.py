...
import unittest
import os
from py_semtools.py_semtools import OboParser

### PREGUNTAR A PEDRO SOBRE ESTAS LINEAS EN RUBY
"""
...
class OBOParserTestCase(unittest.TestCase):
	...
	def setUp(self):
        ...
        #####CREO QUE EN LA DESCRIPCION DE ESTE EJEMPLO HAY ERRORES####
        ...
        ####CREO QUE LA DEFINICION DE ESTE EJEMPLO OBO TIENE ERRORES EN LOS NOMBRES E IDS?####
    ...
    def test_load_file(self):
        ...
        ### PREGUNTAR A PEDRO SOBRE ESTE PRIMER ASSERT DE RUBY
    ...
    def test_expand(self):
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: nil,target_tag: "")) # Nil terms
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: {},target_tag: "")) # Empty terms
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: [],target_tag: "")) # Terms not a hash
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: self.load_Hierarchical[2][:terms],target_tag: nil)) # Nil target
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: self.load_Hierarchical[2][:terms],target_tag: "")) # No/Empty target
		# assertIsNone(Ontology.get_related_ids_by_tag(terms: self.load_Hierarchical[2][:terms],target_tag: 8)) # Target not a string
	...
	def test_dictionaries(self):
		#### PREGUNTAR A PEDRO sobre la presunta clase Ontology
		OboParser.load(Ontology.new(), self.file_Hierarchical["file"], build= True)
		...
		### PREGUNTAR A PEDRO SOBRE LA REGEX
	...
	def test_blacklist(self):
		hierarchical_cutted = Ontology.new(file = self.file_Hierarchical["file"], load_file = True, removable_terms = ["Parental"])
		self.assertEqual(0, hierarchical_cutted.meta["Child2"]["ancestors"])

		####PREGUNTAR A PEDRO como proceder con los casos de Nil de ruby a python. Pasamos a None?
